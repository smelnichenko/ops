...
| output language = the language of the master version it was tuned from — the claims gates compare the output with that master's words, so an Estonian CV needs a reviewed Estonian version of the master, never a translation of the output after the guard. The language a CV is written in is NOT a claim of level: the level is the Languages section, copied from the master by the renderer, and the letter's language line. Never gate a CV's language on the stated level (operator, 2026-09-24) | `[gate: language detection on the output, against the master version's language — any language, not only Estonian letters]` |
