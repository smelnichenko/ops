---
name: full-review
description: Unified review for schnappy repos — runs the built-in code-review and security-review skills, the code-architecture-reviewer agent, a SonarQube server-state check, and conditional domain passes (math verification for control/estimation/geometry/DSP/tuning code; aviation-domain verification for aircraft-modeling/ADS-B/trajectory code) over one scope, then merges everything into a single deduplicated report and auto-fixes every actionable issue it found, in-diff and pre-existing alike (the rest are listed as Remaining). Runs end-to-end without confirmation prompts. Use when the user asks for a full/complete/thorough review of changes, a PR, or a plan doc.
argument-hint: "[low|medium|high|xhigh|max|ultra] [PR# | commit | files... | plan-doc] [--no-simplify]"
---

# Full review

Run up to six independent passes over one scope, merge them into a single report, then apply the automatic follow-ups (steps 4–5) without pausing for confirmation — no AskUserQuestion, no "should I proceed?". The only thing that still requires the user's own keystroke is `ultra` (billed cloud review). Step 4 defines what gets fixed automatically and what lands under **Remaining**.
...
6. **Aviation domain (conditional)** — run only when the scoped diff models aircraft or interprets aviation data: trajectory prediction/tracking of aircraft, ADS-B/Mode-S decoding or field consumption (ground speed/track/heading, baro vs geometric altitude, TC19 subtypes, message cadences), flight-envelope or plausibility constants (speeds, climb/turn rates, maneuver process noise), aircraft classification, ground-observation geometry (angular rates, keyhole, radio horizon), or aerial-vehicle design. If none of that is in scope, skip it and say so in the report. When it applies, spawn an agent (Agent tool) that first reads the `aviation` skill (`ops/.claude/skills/aviation/SKILL.md`, symlinked at `~/.claude/skills/aviation/`) and then adversarially checks the diff's domain assumptions against it: are kinematic bounds physical (derive ω = g·tanφ/v, turn radii, CV-in-turn error — don't trust remembered numbers); do the consumed fields mean what the code assumes (GS vector vs airspeed+heading subtypes, geometric vs barometric altitude for pointing, per-field data ages); does the model survive the hostile vehicle classes (hovering helicopter with GS≈0, glider thermalling at 15–20°/s, Mode-S-only targets with no position); are geometry limits (zenith keyhole, radio horizon, low-elevation data quality) designed for rather than discovered. Findings in the same file:line + failure-scenario format; envelope claims must be computed, not vibed. Both domain passes (5 and 6) can run together when a diff is both mathematical and aeronautical — they check different things (algebra vs domain model).
