...
copies = [t for t in flatten(play_src.get("tasks", []) + play_src.get("pre_tasks", []))
          if isinstance(t.get("ansible.builtin.copy"), dict) and "content" in t["ansible.builtin.copy"]]
if not copies:
...
print("templates:", len(copies), "→", [t.get("name") for t in copies])
