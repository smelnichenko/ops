...
- **Dependency cache:** every Gradle/npm step mounts the PVC `ci-cache` (infra `clusters/production/woodpecker/manifests`) as `volumes: [ci-cache:/cache]` with `GRADLE_USER_HOME=/cache/gradle` / `npm_config_cache=/cache/npm` (+ `npm ci --prefer-offline`); the k8s backend mounts an EXISTING claim by name and needs the repo's **Settings → Trusted → Volumes** flag (linter: `Insufficient trust level to use volumes`). `backend_options.kubernetes.volumes`/`hostPath` is not a Woodpecker option and was silently ignored (depcheck re-downloaded the NVD database nightly since March). Without the cache a build is ~1,300 Nexus requests
...
- **masi:** enforced since 2026-09-21 (masi #18). The pull-request pipeline's `sonar` step waits for the gate, fails when `SONAR_TOKEN`/`SONAR_HOST_URL` are missing (never skips green), and analyses into **`schnappy-masi-pr`** — SonarQube CE has no branch analysis, and two open PRs writing into one project made its issues flip between branches. `schnappy-masi` is written by cd.yaml (main) alone, `wait=false`. Both gates OK on 2026-09-21: 0 issues, 86 % coverage, hotspots reviewed in the code (`// NOSONAR` with the reason, so they hold in either project). `schnappy-masi-pr`'s new code starts at its first analysis (2026-09-20): it judges what pull requests change, `schnappy-masi` judges everything. Lombok builder TYPES named in source must be declared (`public static class XBuilder {}`), or the analyzer loses the method's semantics and reports false dead stores. A new project's first analysis blames back to the root commit; masi's root is an empty commit and jgit needs the empty tree stored (`git hash-object -t tree -w /dev/null`) in a full clone.
...
- **Community Edition usage limit:** 100,000 requests/day (trailing 24 h, aggregated hourly) or 40,000 components; above it Nexus stops caching NEW components: a proxy answers `403 PAYMENT REQUIRED`, the group `404` for anything not already cached, while cached artifacts keep serving 200 — so a Gradle build passes and `npm ci` fails on one new package. Check `GET /service/rest/internal/ui/usage-metrics` (admin) → `requests_per_last_24h`, and `request.log` under `/mnt/data/nexus/log/` for who; it clears by itself once the window drops (2026-09-18: 111k, all CI pods on ten, fixed by the pipelines' `ci-cache` volume)
