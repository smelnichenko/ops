...
**H2 — Close the production identity-trust seam. (Effort: S to flip the flag; M for in-app verification)** Either set `jwt.enabled: true` in `schnappy-production-mesh/values.yaml` so the gateway validates the Keycloak signature and *strips/overwrites* inbound `X-User-*` headers (validating the per-vhost passthrough allowlist), **or** make `GatewayAuthFilter` verify the forwarded token against Keycloak JWKS before trusting headers and parsing roles. Today production trusts unvalidated headers plus an unsigned-token role parse; this fix restores the documented three-layer defense and removes the in-mesh-pod → admin-impersonation path. — **✅ DONE 2026-06-22 (both layers, in prod):** **(B)** `GatewayAuthFilter` in all four services verifies the Keycloak signature against JWKS (`jwk-set-uri`, lazy — no startup dep) before trusting the token, and relays the validated token east-west, so a forged in-mesh token is rejected. **(A)** Istio authz on app :8080 (the `-http` ALLOW policies in schnappy-mesh) now **enforces** the legit-caller allow-list — validated ~a day in dry-run (0 `shadow_denied` on all four apps), then the dry-run annotation removed; post-enforce 0 403s. The impersonation seam is closed at both layers; M2 adds a third on `/internal`.
...
**H4 — Implement scheduled restore verification (Plan 061-C). (Effort: M)** A weekly CronJob that restores a backup to an ephemeral target, runs a k6 smoke test, and emits a `restore_verify_success` metric with a `RestoreVerificationFailing` alert. With single-replica state on local-path disks, backups are the only DR; "untested since March" is unacceptable confidence. The plan is already scoped. — **✅ DONE 2026-06-21:** delivered as the revived Vagrant DR drill (`task dr:drill`) rather than an in-cluster CronJob (it needs Vagrant/libvirt, can't run in CI) — green end-to-end (pod recovery + Velero + **CNPG-barman** postgres restore + offsite 3rd-copy), emitting `restore_verify_success` to a prod pushgateway with the `RestoreVerificationFailing` 35-day-overdue alert. Manual cadence chosen over a scheduler.
...
**M2 — Add defense-in-depth on `/api/internal/**`. (Effort: S–M)** Verify the mTLS SPIFFE principal or a shared internal secret at the app layer so a single Istio policy regression doesn't expose chat/chess membership lookups that gate sub-token minting (currently `permitAll()` with Istio as the only guard). — **✅ DONE 2026-06-22 (in prod):** `InternalCallerFilter` on chat+chess reads the Istio XFCC, takes the spoof-proof **last** element's SPIFFE URI, and 403s unless it equals the admin SA (env `INTERNAL_ALLOWED_CALLER`, templated per-namespace from the same `schnappy.fullname` helper that names the admin SA; **fails open** if unset, so a misconfig degrades to mesh-only, never an outage). Promoted after an adversarial audit confirmed the real Istio 1.25.2 sidecar XFCC (empty Subject DN, bare URI last, no DNS SAN) matches verbatim; prod chat/chess healthy, 0 `/internal` rejections.

**M3 — Add a Keycloak↔admin-DB role reconciliation CronJob + drift alert. (Effort: M)** `KeycloakSyncService` failures are silent; reconcile periodically and emit a sync-failure metric to close the offboarding/role-drift window. — **⏳ IMPLEMENTED, prod-promote deferred 2026-06-22:** built as a `@Scheduled` in-process reconcile (cleaner than a CronJob — reuses the admin's KC client + DB; same idempotent `syncUserRoles` the live path uses) + `admin_keycloak_sync_failures_total` / `admin_keycloak_reconcile_*` metrics + `KeycloakSyncFailing`/`KeycloakReconcileStale` alerts. Code is in test but **dormant** (`@ConditionalOnBean(RealmResource)` only activates where KC-admin is enabled, which test isn't), and prod still runs the pre-M3 image — so it has executed in no environment. Promote needs a **monitored first run** (the new metrics/alerts watch it); deferred for that explicit verification.

**M4 — Codify the out-of-band setup steps. (Effort: M)** Move MinIO bucket creation (`scylla-backups`/`schnappy-backups`) and any ClickHouse schema migration into idempotent, mesh-aware Jobs or documented `task` targets so a rebuild is fully reproducible. — **◑ CORE DONE / residual flagged 2026-06-22** (adversarial audit): the in-cluster setup IS already codified + idempotent — `minio-buckets-job` (`mc mb --ignore-existing` over a per-env bucket list) and `clickhouse-init-job` (+ `IF NOT EXISTS` schema); this review line is stale on those. Three residual **backup-infra hygiene** gaps remain, deliberately NOT rushed (high-stakes, intricate, want Vagrant testing) — tracked as a focused follow-up: (1) in-cluster `schnappy-backups` is a confirmed **orphan** (prod scylla backs up to Pi-MinIO `scylla-backups`); the out-of-band `mc exec` step is dead config. (2) Pi-MinIO bucket creation sits inside a sentinel-gated `start→create→stop` block (`setup-pi-services.yml`), so a wiped bucket doesn't self-heal — a correct fix is a *separate* always-run task against the live VIP (Keepalived-timing-aware), **not** just dropping the guard (which would re-run `stop MinIO` every deploy). (3) the live Velero BSL points at the **in-cluster** `minio-backup`, contradicting comments claiming Pi-MinIO — a duplicated source of truth to reconcile.

**M5 — Introduce automated dependency/version PRs. (Effort: M)** Renovate against the infra repo's Helm `targetRevision` pins, to surface security updates without manual patching of 8+ pinned charts/operators. — **✅ CODE DONE 2026-06-22 (bootstrap pending):** Renovate-for-Forgejo. `monitor/renovate.json` (config:recommended; ignore first-party `git.pmon.dev/schnappy/*` since those promote via GitOps; automerge patch/pin/digest) + `monitor/.woodpecker/renovate.yaml` cron runner (modelled on `depcheck.yaml`) + `ops/docs/renovate-setup.md`. Dormant until the manual bootstrap (a `renovate-bot` Forgejo account, scoped PATs in `woodpecker-ci-secrets`, the `renovate` cron) — Forgejo-admin actions. monitor first, then `RENOVATE_AUTODISCOVER` fan-out.
